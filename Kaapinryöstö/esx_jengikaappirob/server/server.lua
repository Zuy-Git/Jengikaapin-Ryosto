...
AddEventHandler('esx_holdupbank:toofar', function(robb)
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'abyss' then
...
AddEventHandler('esx_holdupbank:rob', function(robb)
	...
	if Banks[robb] then
		...
		if rob == false then
		  ...
		  if xPlayer.getInventoryItem('hammer').count >= 1 then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				TriggerClientEvent('esx:showNotification', source, _U('min_two_abyss') .. Config.NumberOfCopsRequired)
			end
		end
		else
